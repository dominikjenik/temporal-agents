...
TEMPORAL_REQUIREMENTS = [
    ...
    "Intent new_feature routes to ProjectManagerWorkflow → stores HITL task in DB → waits for confirm signal → resolved. UI: task list with 1s polling, detail panel with OK / Comment buttons. Manager returns fixed message; ProjectManager returns JSON {intent, payload}; comment conversation in detail panel via comment signal.",
